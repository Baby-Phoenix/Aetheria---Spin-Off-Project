...
public class GroundChecker : MonoBehaviour
{
   [SerializeField] LayerMask groundLayers;
	...
	private void Update()
	{
		...
		playerPos = new Vector3(cc.transform.position.x, cc.transform.position.y - 1, cc.transform.position.z);
		MainGroundChecker();
...
//This function is only going to check if the player is on ground or not.
	private void MainGroundChecker(){
		...
		Vector3 ray_pos = playerPos + Vector3.up * m_groundCheck.x;
	...
	private bool SlipCheckers(){
		RaycastHit hit;
		Vector3 ray_spwan_pos = playerPos + Vector3.up * m_wallCheck.y; //Y as starting point
	...
	void OnDrawGizmos(){
		Vector3 ray_spwan_pos = playerPos + Vector3.up * m_wallCheck.y;
